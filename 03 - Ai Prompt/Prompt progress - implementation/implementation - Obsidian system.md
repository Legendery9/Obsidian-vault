## Checklist tổng
- [x] Đặt lại tên toàn bộ folder trong `02 - Obsidian system` ✅ 2026-08-18
- [x] Đặt lại tên toàn bộ file `.md` ✅ 2026-08-18
- [x] Hoàn thiện nội dung theo 20/80 cho toàn bộ file ✅ 2026-08-18
- [x] Sắp xếp lại `01 - Features` theo thứ tự Callout → Latex → Markup ✅ 2026-08-18

## Quy tắc áp dụng (tóm tắt)
- **Chuẩn hóa đặt tên:** Sử dụng định dạng `{index} - {context}` cho thư mục và tệp tin.
- **Khử trùng lặp:** Không lặp lại kiến thức, dùng Obsidian wikilink `[[tên-file]]` để tham chiếu.
- **Nguyên tắc 20/80:** Nội dung ngắn gọn, tập trung 20% cốt lõi đem lại 80% giá trị.
- **Định dạng Visual:** Phân tách heading bằng `---`. Sử dụng callouts thích hợp. Ví dụ code dùng block ngôn ngữ tương ứng.

## Nhật ký thay đổi
### [2026-08-18 23:40]
- **Đã làm:**
  - Đổi tên thư mục `Features` → `01 - Features` và `Plugins` → `02 - Plugins`.
  - Tách các chủ đề trong `Features` và lưu trực tiếp dưới dạng tệp tin: `01 - Callout.md`, `02 - Latex.md`, `03 - Markup.md` đúng thứ tự được yêu cầu. Xóa các thư mục con rác.
  - Đổi tên và hoàn thiện nội dung cho 9 plugin trong `02 - Plugins` (từ `01 - Advanced Table.md` đến `09 - Templater.md`) từ trạng thái trống hoàn toàn sang trạng thái tài liệu chi tiết, áp dụng 20/80, bảng biểu trực quan và callouts.

## Trạng thái hiện tại
- Đã hoàn thành: 100% checklist của thư mục `02 - Obsidian system`.
- Đang làm dở / chưa làm tới: Không có.
